...
function App() {
  ...
  const getIndividualBlog = async id => {
    try {
      const response = await fetch("/api/blogRouter/".concat(id));
      const data = await response.json();
      return data;
    } catch (error) {
      console.error(error);
...
function SignUpForm(props) {
  const credentials = [credentials, setCredentials] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)({
    name: '',
    email: '',
    password: ''
  });
...
/******/ })()
;
//# sourceMappingURL=App.1db630eb7a7281ac866e46b0885813f9.js.map